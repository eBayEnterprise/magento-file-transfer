...
class TrueAction_FileTransfer_Test_Helper_FileTest
	extends TrueAction_FileTransfer_Test_Abstract
{
	...
	public function testMvToDirAbsoluteTargetPath()
	{
		...
		$file->expects($this->never())
			->method('mv');
		$this->setExpectedException('TrueAction_FileTransfer_Exception_Configuration', "Path must be relative, given {$targetPath}");
		Mage::helper('filetransfer/file')->mvToDir($srcFile, $targetPath);
	}
	/**
	...
	public function testMvToDirNonExistentFile()
	{
		...
		$file->expects($this->never())
			->method('mv');
		$this->setExpectedException('TrueAction_FileTransfer_Exception_Configuration', "Cannot move {$cleanSrcFile}: No such file or directory");
		Mage::helper('filetransfer/file')->mvToDir($srcFile, $targetPath);
	}
	/**
	...
	public function testMvToDirUnableToCreateTargetDir()
	{
		...
		$this->setExpectedException('TrueAction_FileTransfer_Exception_Configuration', "Target directory '{$targetDir}' does not exist and could not be created.");
		Mage::helper('filetransfer/file')->mvToDir($srcFile, $targetPath);
	}
}